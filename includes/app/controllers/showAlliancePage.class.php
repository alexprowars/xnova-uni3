...
class showAlliancePage extends pageHelper
{
	...
	public function show ()
	{
		...
		if (user::get()->data['ally_id'] == 0)
		{
			...
			$this->display('', _getText('alliance'), false);
		}
		else
		{
			...
			if ($mode == 'exit')
			{
				...
				$this->display($html, 'Выход их альянса', false);
			}
			...
			if ($mode == 'memberslist' || ($mode == 'admin' && $edit == 'members'))
			{
				...
				$this->display('', _getText('Members_list'), false);
			}
			...
			if ($mode == 'circular')
			{
				...
				if (isset($_POST['deletemessages']) && $ally['ally_owner'] == user::get()->data['id'])
				{
					...
					if ($DeleteWhat == 'deleteall')
						db::query("DELETE FROM game_chat WHERE `ally_id` = '" . user::get()->data['ally_id'] . "';");
					elseif ($DeleteWhat == 'deletemarked' || $DeleteWhat == 'deleteunmarked')
					{
						...
						if ($Mess_Array != '')
							db::query("DELETE FROM game_chat WHERE `id` " . (($DeleteWhat == 'deleteunmarked') ? 'NOT' : '') . " IN (" . $Mess_Array . ") AND `ally_id` = '" . user::get()->data['ally_id'] . "';");
					}
				}
				...
				if (isset($_GET['sendmail']) && isset($_POST['text']) && $_POST['text'] != '')
				{
					...
					db::query("INSERT INTO game_chat SET `ally_id` = '" . user::get()->data['ally_id'] . "', `user` = '" . user::get()->data['username'] . "', user_id = " . user::get()->data['id'] . ", `message` = '" . $_POST['text'] . "', `timestamp` = '" . time() . "'");
				...
				$news_count = db::query("SELECT COUNT(*) AS num FROM game_chat WHERE `ally_id` = '" . user::get()->data['ally_id'] . "'", true);
				...
				if ($news_count['num'] > 0)
				{
					...
					$mess = db::query("SELECT * FROM game_chat WHERE `ally_id` = '" . user::get()->data['ally_id'] . "' ORDER BY `id` DESC limit " . (($p - 1) * 20) . ", 20");
			...
			if ($mode == 'admin')
			{
				if (user::get()->isAdmin() && $edit == 'planets')
				{
					...
					$this->display('', "Управление планетами", false);
				}
				...
				if ($edit == 'rights')
				{
					...
					$this->display('', _getText('Law_settings'), false);
				}
				elseif ($edit == 'ally')
				{
					...
					$this->display('', _getText('Alliance_admin'), false);
				}
				elseif ($edit == 'requests')
				{
					...
					$this->display('', _getText('Check_the_requests'), false);
				}
				elseif ($edit == 'name')
				{
					if ($ally['ally_owner'] != user::get()->data['id'] && !$user_admin)
					...
					$this->display('', 'Управление альянсом', false);
				}
				elseif ($edit == 'tag')
				{
					if ($ally['ally_owner'] != user::get()->data['id'] && !$user_admin)
				...
				elseif ($edit == 'give')
				{
					...
					$this->display('', "Передача альянса", false);
				}
			}
			...
			$parse['ally_image'] = ($ally['ally_image'] != '') ? "<tr><th colspan=2 class=nopadding><img src=\"" . $ally['ally_image'] . "\" style=\"max-width:716px\"></th></tr>" : '';
			$parse['range'] = $range;
	...
	public function ainfo ()
	{
		...
		if ($allyrow['ally_image'] != "")
			$allyrow['ally_image'] = "<tr><th colspan=2><img src=\"" . $allyrow['ally_image'] . "\" style=\"max-width:716px\"></th></tr>";
		...
		$this->display('', 'Альянс ' . $allyrow['ally_name'], false);
	}
	...
	public function make ()
	{
		...
		if (isset($_GET['yes']) && $_POST)
		{
			...
			$this->display($this->MessageForm(str_replace('%s', $_POST['atag'], _getText('ally_maked')), str_replace('%s', $_POST['atag'], _getText('alliance_has_been_maked')) . "<br><br>", "?set=alliance", _getText('Ok')), _getText('make_alliance'), false);
		}
		else
		{
			$this->setTemplate('alliance/alliance_make');
			$this->display('', _getText('make_alliance'), false);
		}
	}
	...
	public function search ()
	{
		...
		$this->display('', _getText('search_alliance'), false);
	}
	...
	public function apply ()
	{
		...
		$this->display('', 'Запрос на вступление', false);
	}
