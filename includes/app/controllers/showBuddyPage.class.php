...
class showBuddyPage extends pageHelper
{
	...
	public function show ()
	{
		...
		if (!$parse['a'] && isset($_SESSION['OKAPI']))
		{
			socials::okConnect($_SESSION['OKAPI']['application_key'], $_SESSION['OKAPI']['api_server']);
		
			$friends = socials::okLoad('friends/getAppUsers', Array('session_secret_key' => $_SESSION['OKAPI']['session_secret_key'], 'session_key' => $_SESSION['OKAPI']['session_key']));
		
			$okArray = array();
		
			if (count($friends['uids']) > 0)
			{
				$uInfo = socials::okLoad('users/getInfo', array('uids' => implode(',', $friends['uids']), 'fields' => 'first_name,last_name,name,gender,birthday,age,locale,location,current_location,online,url_profile,pic128x128'));

				if (is_array($uInfo))
				{
					foreach ($uInfo AS $info)
					{
						$okArray[$info['uid']] = $info;
					}
				}
		
				$okFriends = db::extractResult(db::query("SELECT u.id, u.username, u.galaxy, u.system, u.planet, ui.ok_uid, p.name AS planet_name FROM game_users u, game_users_inf ui, game_planets p WHERE p.id = u.id_planet AND u.id = ui.id AND ui.ok_uid IN (".implode(',', $friends['uids']).")"));
			}
			else
				$okFriends = array();
		
			$this->set('okFriends', $okFriends);
			$this->set('okArray', $okArray);
		}
		
		$this->display('', 'Список друзей', false);
	}
}
