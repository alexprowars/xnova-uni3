...
class showOptionsPage extends pageHelper
{
	...
	{
		$inf = db::query("SELECT * FROM game_users_inf WHERE id = " . user::get()->data['id'] . "", true);
		...
		if (isset($_POST['db_password']) && isset($_POST['email']))
		{
			...
			else
			{
				...
				{
					$email = db::query("SELECT id FROM game_users_inf WHERE email = '" . addslashes(htmlspecialchars(trim($_POST['email']))) . "';", true);
	...
	public function change ()
	{
		...
		if (isset($_POST["db_character"]) && trim($_POST["db_character"]) != '' && trim($_POST["db_character"]) != user::get()->data['username'])
		{
		...
		if (isset($_POST['email']) && !is_email($inf['email']) && is_email($_POST['email']))
		{
			...
			$email = db::query("SELECT id FROM game_users_inf WHERE email = '" . $e . "';", true);

			if (!isset($email['id']))
			{
				sql::build()->update('game_users_inf')->setField('email', $e)->where('id', '=', user::get()->getId())->execute();
			}
			else
				$this->message('Данный email уже используется в игре.', 'Hacтpoйки', '?set=options', 3);
		}
		...
		else
		{
			...
			{
				$BuildOnPlanet = db::query("SELECT `id` FROM game_planets WHERE (`b_building` != 0 OR `b_tech` != 0) AND `id_owner` = '" . user::get()->data['id'] . "'");
				...
				{
					$this->message('Heвoзмoжнo включить peжим oтпycкa. Для включeния y вac нe дoлжнo идти cтpoитeльcтвo или иccлeдoвaниe нa плaнeтe.', "Oшибкa", "?set=overview", 5);
				}
		...
		if (user::get()->data['urlaubs_modus_time'] == 0)
		{
			...
			if ($ui_query != '')
			{
				...
				db::query("UPDATE game_users_inf SET" . $ui_query . " WHERE `id` = '" . user::get()->data['id'] . "'");
			}
		...
		if (isset($_POST["db_password"]) && $_POST["db_password"] != "" && $_POST["newpass1"] != "")
		{
			...
			elseif ($_POST["newpass1"] == $_POST["newpass2"])
			{
				...
				db::query("UPDATE game_users_inf SET `password` = '" . $newpass . "' WHERE `id` = '" . user::get()->data['id'] . "' LIMIT 1");
			...
			else
			{
				...
				if (db::num_rows($query) == 0)
				{
					if (preg_match("/^[a-zA-Za-яA-Я0-9_\.\,\-\!\?\*\ ]+$/u", $username) && mb_strlen($username, 'UTF-8') >= 5)
					{
						...
						db::query("UPDATE game_users_inf SET username_last = '" . time() . "' WHERE id = '" . user::get()->data['id'] . "' LIMIT 1");
	...
	public function show ()
	{
		...
		else
		{
			...
			if (user::get()->data['avatar'] != 0)
			{
				if (user::get()->data['avatar'] != 99)
					$parse['avatar'] = "<img src=/images/avatars/" . user::get()->data['avatar'] . ".jpg height='100'><br>";
				else
					$parse['avatar'] = "<img src=/images/avatars/upload/upload_" . user::get()->data['id'] . ".jpg height='100'><br>";
			}
			...
			$parse['identity'] = $inf['identity'];
			$parse['design'] = $inf['design'];
