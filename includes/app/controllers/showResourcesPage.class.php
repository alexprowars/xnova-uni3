...
class showResourcesPage extends pageHelper
{
	...
	public function show ()
	{
		...
		$BuildTemp = app::$planetrow->data['temp_max'];
		$energy_tech = user::get()->data['energy_tech'];
		...
		foreach ($reslist['res'] AS $res)
		{
			...
			$parse[$res.'_storage_bar'] = floor((app::$planetrow->data[$res] / app::$planetrow->data[$res.'_max'] * 100) * 4.25);

			if ($parse[$res.'_storage_bar'] > (100 * 4.25))
			{
				$parse[$res.'_storage_bar'] = 425;
				$parse[$res.'_storage_barcolor'] = '#C00000';
			}
			elseif ($parse[$res.'_storage_bar'] > (80 * 4.25))
