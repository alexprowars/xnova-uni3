...
class showRwPage extends pageHelper
{
	...
	public function show ()
	{
		...
		elseif (!in_array(user::get()->data['id'], $user_list) && user::get()->data['authlevel'] != 3)
		...
		else
		{
			if ($this->page->ajax && $session->isAuthorized())
			{
				...
				if ($user_list[0] == user::get()->data['id'] && $raportrow['no_contact'] == 1)
				{
				...
				$this->display($Page, 'Боевой доклад', false);
			}
			else
			{
				$result = json_decode($raportrow['raport'], true);
