...
function ChatMsg(Time, Player, Msg, Me, My)
{
	...
	for (var i = 0; i < sm_repl.length; i++)
	{
		Msg = Msg.replace(sm_find[i], '<img src="/images/smile/' + sm_repl[i] + '.gif" onclick="S(\'' + sm_repl[i] + '\')" style="cursor:pointer">');
	}
...
function ShowSmiles()
{
	...
	for (var i = 0; i < sm_repl.length; i++)
	{
		str += '<img src=/images/smile/' + sm_repl[i] + '.gif ALT="' + sm_repl[i] + '" onclick="S(\'' + sm_repl[i] + '\')" style="cursor:pointer"> ';
	}
