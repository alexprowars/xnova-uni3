...
{
	$('#menu_links li > a').removeClass('check');
...
function showWindow (title, url, width, height)
{
	...
	$.ajax(
	{
		...
		success: function(html)
		{
			...
			if (width != undefined)
				$('#windowDialog').dialog( "option", "width", width );

			if (height != undefined)
				$('#windowDialog').dialog( "option", "height", height );

			hideLoading();

			$('#windowDialog').html(html);
			$('#windowDialog').dialog("option", "position", { my: "center", at: "center", of: window });
