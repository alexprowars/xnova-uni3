	...
	for (var planet = 1; planet <= 15; planet++)
    {
		...
		if (row[planet] && row[planet]["destruyed"] == 0)
        {
			...
			if (row[planet]['last_update']  < 60 && row[planet]['user_id'] != user['id'])
            {
				if (row[planet]['last_update']  <= 10 && row[planet]['user_id'] != user['id'])
					...
					result += "<span class='star'>("+Math.floor(row[planet]['last_update'])+")</span> ";
			}
		...
		if (row[planet] && row[planet]['user_id'] && row[planet]["destruyed"] == 0)
        {
			...
			if (row[planet]['avatar'] != 0)
            {
                ...
                {
					result += " style=\"background:url(/images/avatars/"+row[planet]['avatar']+".jpg) 50% 50% no-repeat;\"></td>";
				}
                else
                {
					result += " style=\"background:url(/images/avatars/upload/upload_"+row[planet]['user_id']+".jpg) 50% 50% no-repeat;\"></td>";
				}
			}
		...
		if (row[planet] && row[planet]['user_id'] != user['id'])
        {
			if (row[planet]['user_id'] && row[planet]["destruyed"] == 0)
            {
				...
				if (user['spy_sonde'] > 0 && row[planet]['urlaubs_modus_time'] == 0)
					result += "<a href=\"javascript:;\" title=\"Шпионаж\" class=\"tooltip_sticky\" data-tooltip-content='<center><input type=text name=\"spy"+planet+"\" id=\"spy"+planet+"\" value=\""+user['spy']+"\"><br><input type=button value=Отправить onclick=\"Spy("+planet+","+row[planet]['planet_type']+")\"></center>'><span class='sprite skin_e'></span></a>&nbsp;";
...
function PrintSelector(fleet_shortcut, r, l, c)
{
	...
	result += "<form action=\"?set=galaxy&r=1\" method=\"post\" id=\"galaxy_form\">";
	...
	result += "<th><input style=\"min-width: 0;\" name=\"galaxyLeft\" value=\"&lt;-\" onclick=\"galaxy_submit('galaxyLeft')\" type=\"button\"></th>";
	result += "<th><input name=\"galaxy\" value=\""+galaxy+"\" size=\"5\" maxlength=\"3\" tabindex=\"1\" type=\"text\"></th>";
	result += "<th><input style=\"min-width: 0;\" name=\"galaxyRight\" value=\"-&gt;\" onclick=\"galaxy_submit('galaxyRight')\" type=\"button\"></th>";
	result += "</tr></table>";

	result += "</th><th align=\"center\" style=\"vertical-align:middle;\">";
	...
	result += "</select><br><input value=\"Просмотр\" type=\"submit\" style=\"width:250px\"></th><th>";

	result += "<table ><tr>";
	result += "<td class=\"c\" colspan=\"3\">Солнечная система</td></tr><tr>";
	result += "<th><input style=\"min-width: 0;\" name=\"systemLeft\" value=\"&lt;-\" onclick=\"galaxy_submit('systemLeft')\" type=\"button\"></th>";
	result += "<th><input name=\"system\" value=\""+system+"\" size=\"5\" maxlength=\"3\" tabindex=\"2\" type=\"text\"></th>";
	result += "<th><input style=\"min-width: 0;\" name=\"systemRight\" value=\"-&gt;\" onclick=\"galaxy_submit('systemRight')\" type=\"button\"></th>";
	result += "</tr></table>";

	result += "</th></tr></table></form><div class=\"separator\"></div>";
