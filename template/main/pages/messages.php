			...
			<table class="table">
				...
				<? foreach ($parse['list'] AS $list): ?>
					<tr>
						...
						<th><?=datezone("d-m H:i:s", $list['message_time']) ?></th>
						<th><?=$list['message_from'] ?></th>
						<th>
					...
					<tr>
						<td style="background-color:<?=_getText('mess_background', $list['message_type']) ?>;" colspan="4" class="b">
							<? if ($list['message_type'] == 1 && user::get()->getUserOption('bb_parser')): ?>
								<span id="m<?=$list['message_id'] ?>"></span>
								<script type="text/javascript">Text('<?=str_replace(array("\r\n", "\n", "\r"), '', stripslashes($list['message_text'])) ?>', 'm<?=$list['message_id'] ?>');</script>
							<? else: ?>
